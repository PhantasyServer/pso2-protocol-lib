...
use crate::{
    connection::{
        conn_impl::{ConnectionReader, ConnectionWriter},
        PrivateKey,
    },
...
impl ProxyConnection {
    ...
    pub fn read_packet(&mut self) -> std::io::Result<ProxyPacket> {
        ...
        let mut packets = ProxyPacket::read(&data, self.packet_type)?;
        self.read_packets.append(&mut packets);
        let mut packet = self.get_one_packet();
        if let ProxyPacket::EncryptionRequest(data) = &mut packet {
    ...
    pub fn write_packet(&mut self, packet: &ProxyPacket) -> std::io::Result<()> {
    ...
    #[cfg(feature = "tokio")]
    #[cfg_attr(docsrs, doc(cfg(feature = "tokio")))]
    pub async fn write_packet_async(&mut self, packet: &ProxyPacket) -> std::io::Result<()> {
        self.prepare_data(packet)?;
        self.write.flush_async(&mut self.stream).await?;
        Ok(())
    }
    fn prepare_data(&mut self, packet: &ProxyPacket) -> std::io::Result<()> {
        let _packet = if let ProxyPacket::EncryptionRequest(data) = packet {
                ...
                && !matches!(&self.in_keyfile, PrivateKey::None)
            {
                ...
                new_packet.rsa_data = encrypt(&data.rsa_data, &self.out_keyfile)?;
...
impl ProxyRead {
    ...
    /// # Note
    ///
    /// If the encryption was not yet setup (i.e [`ProxyPacket::EncryptionResponse`] was not
    ...
    #[cfg(feature = "tokio")]
    #[cfg_attr(docsrs, doc(cfg(feature = "tokio")))]
    pub async fn read_packet_async(&mut self) -> std::io::Result<ProxyPacket> {
        if !self.read_packets.is_empty() {
            return Ok(self.get_one_packet());
        }
        let data = loop {
    ...
    fn parse_packet(&mut self, data: &[u8]) -> std::io::Result<ProxyPacket> {
        let mut packets = ProxyPacket::read(data, self.packet_type)?;
