...
use syn::{
    parse::Parse, parse_macro_input, punctuated::Punctuated, spanned::Spanned, Attribute, Data,
    DataStruct, DeriveInput, GenericArgument, Ident, LitInt, MetaList, PathArguments, Token, Type, Fields, DataEnum, Expr, Lit,
};

// I don't know what is worse: a 200+ line impl or a hard to read macro
// But i think macros make life easier
#[proc_macro_derive(
    PacketReadWrite,
    attributes(
        Id,
...
fn packet_deriver(ast: &syn::DeriveInput) -> syn::Result<TokenStream> {
    let name = &ast.ident;
...
fn helper_deriver(ast: &syn::DeriveInput) -> syn::Result<TokenStream> {
    let name = &ast.ident;
...
fn parse_enum_field(read: &mut TS2, write: &mut TS2, data: &DataEnum, repr_type: EnumRepr) -> syn::Result<()> {
        ...
        for attr in &variant.attrs {
            match &attr.meta {
                ...
                syn::Meta::Path(path) => {
                    ...
                    get_attrs(&mut settings, &string, None, &mut quote!{}, &mut quote!{}, name)?;
                }
                syn::Meta::List(_) => {}
            }
        }
...
fn parse_struct_field(read: &mut TS2, write: &mut TS2, data: &DataStruct) -> syn::Result<()> {
    ...
    if let Fields::Unnamed(fileds) = &data.fields {
        ...
        for (id, field) in fileds.unnamed.iter().enumerate() {
            ...
            check_syn_type(&field.ty, read, &mut tmp_write, &varname, &Settings::default(), false)?;
        }
...
fn check_code_type(
    ...
    match string.as_str() {
        ...
        "f64" => {
            ...
            write.extend(quote! {writer.write_f64::<LittleEndian>(#write_name).unwrap();});
        },
        "Ipv4Addr" => {
            read.extend(quote! {
